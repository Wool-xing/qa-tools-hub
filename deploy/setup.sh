...
# 在仓库根目录运行: sudo ./deploy/setup.sh your-domain.com
set -e

APP_DIR="/opt/qa-tools"
DOMAIN="${1:-localhost}"
...
echo "[2/6] 创建目录..."
sudo mkdir -p "$APP_DIR" "$APP_DIR/backend/data" "$APP_DIR/backend/backups"
sudo cp -r . "$APP_DIR"

# 3. Setup Python venv (backend 为工作目录)
echo "[3/6] 配置Python环境..."
...
if ! grep -q "SECRET_KEY=." "$APP_DIR/backend/.env" 2>/dev/null; then
    echo "[4/6] 生成密钥..."
    SECRET_KEY=$(python3 -c "import secrets; print(secrets.token_urlsafe(32))")
    sed -i "s/SECRET_KEY=/SECRET_KEY=$SECRET_KEY/" "$APP_DIR/backend/.env"
fi
